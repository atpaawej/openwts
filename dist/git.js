 ...
 */
export async function getCurrentBranch(system, cwd) {
 ...
 */
export async function getDefaultBranch(system, cwd) {
    // Try origin/HEAD first
    const originResult = await system.exec('git', ['symbolic-ref', 'refs/remotes/origin/HEAD'], { cwd });
 ...
 */
export async function getStatus(system, cwd) {
    const statusRes = await system.exec('git', ['status', '--porcelain'], { cwd });
    const isDirty = statusRes.stdout.trim().length > 0;
    const unpushedRes = await system.exec('git', ['log', '@{u}..HEAD', '--oneline'], { cwd });
 ...
 */
export async function getRepoRoot(system) {
