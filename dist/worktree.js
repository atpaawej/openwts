...
export function createWorktree(system) {
...
class RealWorktree {
    ...
    async repoRoot() {
        ...
        if (result.exitCode !== 0) {
            throw new OpenwtError('Not in a git repository', 'Run openwts from inside a git repository');
        }
    ...
    async getPath(name) {
        ...
        if (fuzzy)
            return fuzzy.path;
        throw new OpenwtError(`Worktree "${name}" not found`, `Create it first: openwts create ${name}`);
    }
    async create(name, base) {
        // Validate name
        if (!/^[\w.-]+$/.test(name)) {
            throw new OpenwtError(`Invalid worktree name "${name}"`, 'Use only letters, numbers, hyphens, dots, and underscores');
        }
        const root = await this.repoRoot();
        const wtPath = `${root}/${WORKTREE_DIR}/${name}`;
        // Check if worktree already exists
        if (await this.system.exists(wtPath)) {
            throw new OpenwtError(`Worktree "${name}" already exists`, `Use: openwts run ${name}`);
        }
        ...
        if (branchCheck.exitCode !== 0) {
            ...
            if (retryCheck.exitCode !== 0) {
                throw new OpenwtError(`Base branch "${baseBranch}" not found`, 'Make sure the branch exists locally or on origin');
            }
        }
        // Create the worktree
        const addResult = await this.system.exec('git', ['worktree', 'add', '-b', name, wtPath, baseBranch], { cwd: root });
        if (addResult.exitCode !== 0) {
            // Check for specific errors
            const stderr = addResult.stderr;
            if (stderr.includes('already exists')) {
                throw new OpenwtError(`Branch "${name}" already exists`, 'Use a different name or remove the existing branch: git branch -D ' + name);
            }
    ...
    async remove(name, force) {
        ...
        if (path.replace(/\\/g, '/') === root.replace(/\\/g, '/')) {
            throw new OpenwtError('Cannot remove the main worktree', 'You can only remove worktrees created by openwts');
        }
        ...
        const unpushedRes = await this.system.exec('git', ['log', '@{u}..HEAD', '--oneline'], { cwd: path });
        ...
        if (force) {
            ...
            if (rmResult.exitCode !== 0) {
                throw new OpenwtError(`Cannot remove "${name}"`, `${rmResult.stderr.trim()}\nUse --force to override`);
            }
        }
        ...
        for (const wt of toRemove) {
            try {
                ...
                if (e instanceof OpenwtError) {
                    throw new OpenwtError(`Failed to remove "${wt.name}"`, e.message);
                }
                throw e;
            }
        }
