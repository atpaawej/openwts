...
export const removeCommand = {
    ...
    async run(args, ctx) {
        ...
        // Check unpushed
        const unpushedRes = await ctx.system.exec('git', ['log', '@{u}..HEAD', '--oneline'], { cwd: path });
        const hasUnpushed = unpushedRes.stdout.trim().length > 0;
        if (hasUnpushed) {
            ctx.output.warn(`"${name}" has unpushed commits`);
        }
        if ((isDirty || hasUnpushed) && !force) {
            throw new OpenwtError(`"${name}" has pending changes`, 'Commit, push, or stash first, or use --force');
        }
